          ...
          <div className="flex items-center space-x-4">
            ...
            <div className="relative">
              <button className="flex items-center space-x-2 focus:outline-none">
                <div className="h-8 w-8 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white flex items-center justify-center">
                  <span className="text-sm font-medium">E</span>
                </div>
                <span className="hidden md:inline-block text-sm font-medium text-gray-700">Employer</span>
              </button>
            </div>
          </div>
        </div>
      ...
      <div className={`hidden lg:flex flex-col flex-shrink-0 transition-all duration-300 ${collapsed ? 'w-20' : 'w-64'}`}>
        <div className="flex flex-col h-full border-r border-gray-200 bg-white">
          ...
          <nav className="mt-5 flex-1 px-2 space-y-1 overflow-y-auto">
            ...
            {/* Removed Post New Job button as per user request */}
          </nav>
          
          {/* User profile section */}
          <div className="flex-shrink-0 border-t border-gray-200 p-4">
            <div className="flex items-center">
              <div className="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white flex items-center justify-center flex-shrink-0">
                <span className="text-sm font-medium">E</span>
              </div>
              {!collapsed && (
                <div className="ml-3 min-w-0">
                  <p className="text-sm font-medium text-gray-900 truncate">Employer</p>
                  <p className="text-xs text-gray-500 truncate">employer@example.com</p>
                </div>
              )}
              {!collapsed && (
      ...
      <div className={`fixed inset-0 z-40 lg:hidden ${sidebarOpen ? 'flex' : 'hidden'}`}>
        ...
        <div className="relative flex-1 flex flex-col max-w-xs w-full bg-white">
          ...
          <div className="flex-shrink-0 flex border-t border-gray-200 p-4">
            <div className="flex items-center">
              <div className="h-10 w-10 rounded-full bg-gradient-to-r from-blue-500 to-indigo-600 text-white flex items-center justify-center">
                <span className="text-sm font-medium">E</span>
              </div>
              <div className="ml-3">
                <p className="text-sm font-medium text-gray-900">Employer</p>
                <p className="text-xs text-gray-500">employer@example.com</p>
              </div>
              <button className="ml-auto p-1 text-gray-400 hover:text-gray-500 focus:outline-none">
                <ArrowRightStartOnRectangleIcon className="h-5 w-5" />
              </button>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="mt-1 text-sm text-gray-500">
                    {location.pathname.endsWith('dashboard') && 'Overview of your company and job postings'}
                    {location.pathname.endsWith('jobs') && 'Manage your job postings and applications'}
                    {location.pathname.endsWith('candidates') && 'View and manage job candidates'}
                    {location.pathname.endsWith('company') && 'Manage your company profile and settings'}
                    {location.pathname.endsWith('analytics') && 'View analytics and insights'}
                  </p>
                </div>
                <div className="flex space-x-3">
                  {location.pathname.endsWith('jobs') && (
                    <button className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500">
