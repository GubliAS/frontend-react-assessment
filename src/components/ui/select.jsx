...
const SelectTrigger = forwardRef(({ className = "", children, ...props }, ref) => (
  <SelectPrimitive.Trigger
    ...
    className={classNames(
      "flex h-10 w-full items-center justify-between rounded-md bg-white shadow-sm px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1",
      className
    )}
    {...props}
  >
...
const SelectContent = forwardRef(({ className = "", children, position = "popper", ...props }, ref) => (
  <SelectPrimitive.Portal>
    <SelectPrimitive.Content
      ref={ref}
      className={classNames(
        "relative z-50 max-h-96 min-w-[8rem] overflow-hidden bg-white rounded-md  bg-popover text-popover-foreground shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out",
        position === "popper" &&
