  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold text-[var(--ebony-900)]">Assessment History</h1>
          <p className="text-[var(--ebony-600)]">Track your progress and performance over time</p>
        </div>
        <button onClick={exportHistory} className="flex items-center gap-2 border rounded px-3 py-2 hover:bg-gray-50 text-[var(--ebony-900)]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        <Card className="bg-white text-card-foreground border border-gray-100 shadow-sm text-center p-4">
          <CardHeader className="p-4">
            <Trophy className="w-8 h-8 text-yellow-500 mx-auto mb-2" />
            <div className="text-2xl font-bold">{totalAssessments}</div>
            <p className="text-sm text-[var(--ebony-600)]">Total Assessments</p>
          </CardHeader>
        </Card>
        ...
        <Card className="bg-white text-card-foreground border border-gray-100 shadow-sm text-center p-4">
          <CardHeader className="p-4">
            ...
            <p className="text-sm text-[var(--ebony-600)]">Average Score</p>
          </CardHeader>
        </Card>
        ...
        <Card className="bg-white text-card-foreground border border-gray-100 shadow-sm text-center p-4">
          <CardHeader className="p-4">
            ...
            <p className="text-sm text-[var(--ebony-600)]">Highest Score</p>
          </CardHeader>
        </Card>
        ...
        <Card className="bg-white text-card-foreground border border-gray-100 shadow-sm text-center p-4">
          <CardHeader className="p-4">
            ...
            <p className="text-sm text-[var(--ebony-600)]">Total Points</p>
          </CardHeader>
        </Card>
      </div>

      {/* Charts */}
      <Card className="bg-white text-card-foreground border border-gray-100 shadow-sm p-4">
        <CardHeader className="p-4">
          <h2 className="font-semibold mb-1 text-[var(--ebony-900)]">Score Progress Over Time</h2>
          <p className="text-sm text-[var(--ebony-600)] mb-4">Recent assessment performance</p>
        </CardHeader>
      ...
      <Card className="bg-white text-card-foreground border border-gray-100 shadow-sm p-4">
        <CardHeader className="p-4">
          <h2 className="font-semibold mb-1 text-[var(--ebony-900)]">Average Performance by Type</h2>
          <p className="text-sm text-[var(--ebony-600)] mb-4">Grouped average scores</p>
        </CardHeader>
        <CardContent className="p-4">
          <ResponsiveContainer width="100%" height={300}>
      ...
      <div className="space-y-4">
        {historyData.map((assessment) => (
          <Card key={assessment.id} className="bg-white border border-gray-100 rounded p-4 hover:shadow-md transition">
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  <div className="text-2xl">{getTypeIcon(assessment.type)}</div>
                  <div>
                    <h3 className="font-semibold text-[var(--ebony-900)]">{assessment.assessmentTitle}</h3>
                    <div className="flex gap-2 mt-1">
                      <span className={`px-2 py-1 rounded text-xs ${getDifficultyColor(assessment.difficulty)}`}>{assessment.difficulty}</span>
                      <span className="px-2 py-1 rounded text-xs border text-[var(--ebony-900)]">{assessment.type}</span>
                    </div>
                  </div>
                </div>
                <div className="text-right">
                  <div className={`text-2xl font-bold ${getScoreColor(assessment.score)}`}>{assessment.score}%</div>
                  <div className="text-sm text-[var(--ebony-600)]">{assessment.correctAnswers}/{assessment.totalQuestions} correct</div>
                </div>
              </div>

              <div className="grid grid-cols-3 gap-4 mt-4 pt-4 border-t text-sm text-[var(--ebony-600)]">
              ...
              <div className="mt-4">
                <button className="flex items-center gap-2 border rounded px-3 py-1 text-sm hover:bg-gray-50 text-[var(--ebony-900)]">
